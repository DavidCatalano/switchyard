...
- MCP Playwright: Prefer the MCP Playwright tool for browser checks and app
  verification. Do not add Playwright configs/tests or modify `package.json`
  for Playwright without explicit approval.
- E2E Runs: Avoid introducing local Playwright scaffolding by default. Use
  MCP-driven checks or manual verification.
...
└── sep/
    ...
    ├── SEP-000-04-[CONTEXT|CONSULT|PR|REVIEW]-feature-name.md
    └── SEP-000-##-[CONTEXT|CONSULT|PR|REVIEW]-feature-name.md
```
...
- **DOC-TYPE**: `PRD`, `PLAN`, `DEVLOG`, `CONTEXT`, `CONSULT`, `ADR`, `PR`, or
  `BUG`.
...
| ADR | `switchyard-internal/process/DEV_TEMPLATE_ADR.md` | - | As-needed | High-impact cross-effort decisions only |
| PR | `switchyard-internal/process/DEV_TEMPLATE_PR.md` | As-needed | As-needed | Draft summary; final PR lives in GitHub |
